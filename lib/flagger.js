...
class Flagger {
  ...
  groupAndOrderData(data) {
    ...
    let groupedBy;
    if (typeof this.config.groupBy === 'string') {
      groupedBy = _.groupBy(data, this.config.groupBy);
    } else if (typeof this.config.groupBy === 'object') {
      groupedBy = _.groupBy(data, (d) => {
        const groupParentKey = Object.keys(this.config.groupBy)[0];
        const groupByNestedFields = this.config.groupBy[groupParentKey];
        return _.map(groupByNestedFields, f => d[groupParentKey][f]);
      });
    }
